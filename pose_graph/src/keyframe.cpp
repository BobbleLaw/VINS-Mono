...
{
	const BriefExtractor extractor{BRIEF_PATTERN_FILE.c_str()};
								...
								const std::vector<cv::KeyPoint> &keypoints_old_norm)
{
	for (int i = 0; i < (int)window_brief_descriptors.size(); i++)
	{
		...
		status.push_back(searchInArea(window_brief_descriptors[i], descriptors_old, keypoints_old, keypoints_old_norm, pt, pt_norm));
...
Eigen::Vector3d KeyFrame::getLoopRelativeT() const
{
	return {loop_info(0), loop_info(1), loop_info(2)};
}

Eigen::Quaterniond KeyFrame::getLoopRelativeQ() const
{
	return {loop_info(3), loop_info(4), loop_info(5), loop_info(6)};
}

double KeyFrame::getLoopRelativeYaw() const
{
	return loop_info(7);
}
